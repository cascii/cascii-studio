...
async fn create_preview(app: tauri::AppHandle, request: CreatePreviewRequest) -> Result<database::Preview, String> {
    ...
    let source_name = input_path.file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("video");
    let random_suffix = generate_random_suffix();
    ...
    let status = Command::new(ffmpeg_cmd)
        .arg("-ss").arg(format!("{}", request.timestamp))
        .arg("-i").arg(&request.video_path)
        .arg("-vframes").arg("1")
        .arg("-y")
        .arg(&temp_frame_path)
        .stdout(Stdio::null())
    ...
    converter.convert_image(&temp_frame_path, &output_txt_path, &conv_opts)
...
#[derive(serde::Deserialize)]
struct DeletePreviewRequest {
    preview_id: String,
    folder_path: String,
}

#[tauri::command]
fn delete_preview(request: DeletePreviewRequest) -> Result<(), String> {
    // Delete the folder
